    ...
    >
      ...
      >
        ...
        <div className="relative w-full max-w-[380px] rounded-3xl bg-cave-stone/90 p-5 pb-6 shadow-[0_20px_60px_rgba(0,0,0,0.6)]">
          ...
          {/* Icons row: Save + WhatsApp + Instagram + X + Copy */}
          <div className="flex items-center gap-2 mb-3">
            {user && (
              <button onClick={handleToggleSave} disabled={savingInProgress} className={`w-9 h-9 flex items-center justify-center rounded-full transition-colors disabled:opacity-50 ${saved ? "bg-cave-white text-cave-black" : "bg-cave-rock hover:bg-cave-ash text-cave-smoke hover:text-cave-white"}`} aria-label={saved ? "Saved" : "Save"}>
            ...
            <a href={`https://wa.me/?text=${encodeURIComponent((flyer.title || "Check this out") + " — " + (typeof window !== "undefined" ? window.location.origin : "") + "/flyer/" + flyer.id)}`} target="_blank" rel="noopener noreferrer" className="w-9 h-9 flex items-center justify-center rounded-full bg-cave-rock hover:bg-cave-ash transition-colors" aria-label="WhatsApp">
            ...
            <button onClick={handleShare} className="w-9 h-9 flex items-center justify-center rounded-full bg-cave-rock hover:bg-cave-ash transition-colors" aria-label="Instagram">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-[#E4405F]"><rect x="2" y="2" width="20" height="20" rx="5" ry="5" /><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z" /><line x1="17.5" y1="6.5" x2="17.51" y2="6.5" /></svg>
            </button>
            <a href={`https://twitter.com/intent/tweet?text=${encodeURIComponent((flyer.title || "Check this out") + " — Found on Caves")}&url=${encodeURIComponent((typeof window !== "undefined" ? window.location.origin : "") + "/flyer/" + flyer.id)}`} target="_blank" rel="noopener noreferrer" className="w-9 h-9 flex items-center justify-center rounded-full bg-cave-rock hover:bg-cave-ash transition-colors" aria-label="X">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" className="text-cave-white"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
            </a>
            <button onClick={async () => { try { await navigator.clipboard.writeText(`${window.location.origin}/flyer/${flyer.id}`); setShareToast(true); setTimeout(() => setShareToast(false), 2000); } catch {} }} className="w-9 h-9 flex items-center justify-center rounded-full bg-cave-rock hover:bg-cave-ash transition-colors" aria-label="Copy link">
            ...
            {creator ? (
              <Link href={`/profile/${creator.username}`} className="flex items-center gap-2 group">
                <div className="w-7 h-7 rounded-full overflow-hidden bg-cave-rock border border-cave-ash shrink-0">
                  {creator.avatar_url ? (
                    <Image src={creator.avatar_url} alt={creator.username} width={28} height={28} className="w-full h-full object-cover" unoptimized />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center">
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-cave-smoke"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /></svg>
                    </div>
                  )}
                </div>
                <span className="text-sm text-cave-fog group-hover:text-cave-white transition-colors">@{creator.username}</span>
              </Link>
            ) : <div />}
            {viewCount > 0 && (
