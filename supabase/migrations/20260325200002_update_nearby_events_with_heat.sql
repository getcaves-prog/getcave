-- Update get_nearby_events to include heat_count and expires_at
-- Must DROP first because return type changed (added heat_count and expires_at columns)
DROP FUNCTION IF EXISTS public.get_nearby_events(DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION, TEXT, INTEGER, INTEGER);
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(e.heat_count, 0) AS heat_count,
    e.expires_at,
